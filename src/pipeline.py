...
Two generation modes live here:
  generate_sample()      -- uniformly-random crop placement (with an
                             optional bias toward mat/strip boundaries).
  generate_sample_tiered()-- four explicit, labelled difficulty tiers
                             (easy/medium/hard/very_hard) that control
                             landmark presence/strength and noise
                             independently, so failures can be attributed
                             to a specific cause instead of an unlabelled
                             mix. Every sample also records an FFT-
                             autocorrelation periodicity_score for its
                             reference crop. dataset_generator.py uses
                             this mode.
"""
...
from src.patterns.landmarks import inject_landmarks, periodicity_score
...
class GenerationParams:
    ...
    # (generate_sample() only -- generate_sample_tiered() controls crop
    # placement explicitly via its tier definitions instead.)
...
def generate_sample(
    ...
    """Uniformly-random (optionally boundary-biased) crop placement, no
    difficulty tiers or injected landmarks. Used by evaluate_improved.py
    and generate_family_dataset.py."""
    if preset_overrides:
...
# ── Tiered / landmark-aware generation ──────────────────────────────────────

TIERS = {
    # glyph_size_px is sized in REFERENCE-image pixels, but localize.py
    # resizes the reference template down ~10x to match the search image
    # scale before ZNCC. A glyph must survive THAT resize, not just be
    # visible in the reference crop -- so glyph_size_px needs to be large
    # enough that size/10 is still well above the ~2-3px Nyquist floor
    # that already kills near-pitch routing strips (see the dram_dense
    # failure case in the failure-analysis section of the README).
    # ~150px reference -> ~15px post-resize is a comfortable margin;
    # ~110px -> ~11px is still safe but smaller/lower-contrast, matching
    # the intended "weak landmark" semantics.
    "easy": dict(
        landmark_count=6, landmark_strength=0.9, glyph_size_px=150,
        dose_search=700.0, shear_amplitude_px=0.6, drift_jitter_px=0.25,
        speckle_sigma=0.02, salt_pepper_prob=0.0005,
    ),
    "medium": dict(
        landmark_count=6, landmark_strength=0.35, glyph_size_px=110,
        dose_search=180.0, shear_amplitude_px=1.5, drift_jitter_px=0.6,
        speckle_sigma=0.06, salt_pepper_prob=0.002,
    ),
    "hard": dict(
        landmark_count=0, landmark_strength=0.0, glyph_size_px=0,
        dose_search=180.0, shear_amplitude_px=1.5, drift_jitter_px=0.6,
        speckle_sigma=0.06, salt_pepper_prob=0.002,
    ),
    "very_hard": dict(
        landmark_count=0, landmark_strength=0.0, glyph_size_px=0,
        dose_search=35.0, shear_amplitude_px=3.5, drift_jitter_px=1.6,
        speckle_sigma=0.25, salt_pepper_prob=0.01,
    ),
}
TIER_NAMES = list(TIERS.keys())


def _mat_interior_origin(mat_rects, rng, margin_px, avoid_points=(), avoid_radius=0):
    """Sample a REFERENCE_SIZE_PX crop origin lying fully inside one mat
    rect (i.e. NOT straddling a strip boundary), optionally rejecting
    origins whose crop would contain any point in avoid_points."""
    candidates = [r for r in mat_rects if r[2] >= REFERENCE_SIZE_PX + 2 * margin_px
                  and r[3] >= REFERENCE_SIZE_PX + 2 * margin_px]
    if not candidates:
        return None
    for _ in range(40):
        rx, ry, rw, rh = candidates[int(rng.integers(0, len(candidates)))]
        x0 = int(rng.integers(rx + margin_px, rx + rw - REFERENCE_SIZE_PX - margin_px + 1))
        y0 = int(rng.integers(ry + margin_px, ry + rh - REFERENCE_SIZE_PX - margin_px + 1))
        if avoid_radius > 0 and avoid_points:
            cx, cy = x0 + REFERENCE_SIZE_PX / 2.0, y0 + REFERENCE_SIZE_PX / 2.0
            if any(np.hypot(cx - px, cy - py) < avoid_radius for px, py in avoid_points):
                continue
        return x0, y0
    return None


def _landmark_centered_origin(landmark_pts, rng, jitter=None, safety_px=40):
    """Pick a crop origin so the chosen landmark lands inside the crop
    (not just near it). Returns (origin, chosen_landmark) or (None, None)
    if no landmarks were given."""
    if not landmark_pts:
        return None, None
    if jitter is None:
        jitter = max(REFERENCE_SIZE_PX / 2.0 - safety_px, 0.0)
    max_offset = FINE_CANVAS_SIZE_PX - REFERENCE_SIZE_PX
    chosen = landmark_pts[int(rng.integers(0, len(landmark_pts)))]
    lx, ly, _ = chosen
    x0 = lx - REFERENCE_SIZE_PX / 2.0 + rng.uniform(-jitter, jitter)
    y0 = ly - REFERENCE_SIZE_PX / 2.0 + rng.uniform(-jitter, jitter)
    x0 = int(np.clip(x0, 0, max_offset))
    y0 = int(np.clip(y0, 0, max_offset))
    return (x0, y0), chosen


def generate_sample_tiered(
    architecture: str,
    tier: str,
    rng: np.random.Generator,
    base_params: GenerationParams | None = None,
) -> dict:
    """Explicit-difficulty generation used by dataset_generator.py. See
    TIERS above and the README's Difficulty Tiers section."""
    if tier not in TIERS:
        raise ValueError(f"Unknown tier '{tier}'. Available: {TIER_NAMES}")
    cfg = TIERS[tier]
    base_params = base_params or GenerationParams()
    params = replace(
        base_params,
        dose_search=cfg["dose_search"],
        shear_amplitude_px=cfg["shear_amplitude_px"],
        drift_jitter_px=cfg["drift_jitter_px"],
        speckle_sigma=cfg["speckle_sigma"],
        salt_pepper_prob=cfg["salt_pepper_prob"],
    )

    preset = get_preset(architecture)
    zone_result = generate_zone_canvas(
        FINE_CANVAS_SIZE_PX, preset["kind"], params.collapse_threshold_nm, rng,
    ...
    landmarks = inject_landmarks(
        fine_canvas, mat_rects, rng,
        count=cfg["landmark_count"], strength=cfg["landmark_strength"],
        glyph_size_px=cfg["glyph_size_px"],
    )

    origin = None
    chosen_landmark = None
    if landmarks:
        # keep the full glyph inside the crop, not just its centre point
        safety_px = cfg["glyph_size_px"] // 2 + 30
        origin, chosen_landmark = _landmark_centered_origin(landmarks, rng, safety_px=safety_px)
    if origin is None:
        origin = _mat_interior_origin(
            mat_rects, rng, margin_px=20,
            avoid_points=[(lx, ly) for lx, ly, _ in landmarks],
            avoid_radius=250 if not landmarks else 0,
        )
    if origin is None:
        # fallback: any crop that fits (should be rare -- tiny mats/huge strips)
        max_offset = FINE_CANVAS_SIZE_PX - REFERENCE_SIZE_PX
        origin = (int(rng.integers(0, max_offset + 1)), int(rng.integers(0, max_offset + 1)))
    x0, y0 = origin

    crop = fine_canvas[y0:y0 + REFERENCE_SIZE_PX, x0:x0 + REFERENCE_SIZE_PX]

    reference_img = sem_imaging.image_reference(
        crop, pixel_size_nm=PIXEL_SIZE_REF_NM, spot_size_nm=params.beam_spot_size_nm,
        dose=params.dose_reference, rng=rng,
        detector_noise_sigma=params.detector_noise_sigma_ref,
        drift_jitter_px=params.drift_jitter_px * 0.2,
        astigmatism_ratio=params.astigmatism_ratio,
        vignette_strength=params.vignette_strength * 0.5, gamma=params.gamma,
        barrel_distortion_k=params.barrel_distortion_k * 0.3,
        charging_streak_prob=params.charging_streak_prob,
        charging_streak_intensity=params.charging_streak_intensity,
    ...
    search_img = sem_imaging.image_search(
        fine_canvas, pixel_size_ref_nm=PIXEL_SIZE_REF_NM,
        pixel_size_search_nm=PIXEL_SIZE_SEARCH_NM, spot_size_nm=params.beam_spot_size_nm,
        dose=params.dose_search, rng=rng, shear_amplitude_px=params.shear_amplitude_px,
        drift_jitter_px=params.drift_jitter_px,
        detector_noise_sigma=params.detector_noise_sigma_search,
        astigmatism_ratio=params.astigmatism_ratio, vignette_strength=params.vignette_strength,
        gamma=params.gamma, barrel_distortion_k=params.barrel_distortion_k,
        charging_streak_prob=params.charging_streak_prob,
        charging_streak_intensity=params.charging_streak_intensity,
        speckle_sigma=params.speckle_sigma, salt_pepper_prob=params.salt_pepper_prob,
    )
    ...
    strip_rects = zone_result.get("strip_rects", [])
    on_boundary = any(
        x0 < sx + sw and x0 + REFERENCE_SIZE_PX > sx and
        y0 < sy + sh and y0 + REFERENCE_SIZE_PX > sy
        for sx, sy, sw, sh in strip_rects
    )
    landmark_in_crop = any(
        x0 <= lx <= x0 + REFERENCE_SIZE_PX and y0 <= ly <= y0 + REFERENCE_SIZE_PX
        for lx, ly, _ in landmarks
    )

    # Reference-local (crop-relative) landmark bbox, if the crop was
    # deliberately centred on one. Recorded for dataset diagnostics only --
    # localize.py's saliency detector never reads this, it re-derives
    # candidate patches from the reference image alone at inference time.
    landmark_ref_x = landmark_ref_y = landmark_size = None
    if chosen_landmark is not None:
        lx, ly, lsize = chosen_landmark
        landmark_ref_x = lx - x0
        landmark_ref_y = ly - y0
        landmark_size = lsize
    ...
    return {
        ...
        "landmark_in_crop": landmark_in_crop,
        "landmark_strength": cfg["landmark_strength"],
        "landmark_ref_x": landmark_ref_x,
        "landmark_ref_y": landmark_ref_y,
        "landmark_size": landmark_size,
        "on_boundary": on_boundary,
        "periodicity_score": periodicity_score(crop),
        "params": params.as_dict(),
    }
